...
def createMountain(x, y, xc, yc):
    ...
    for xi in range(nx):
        for yi in range(ny):
            rm = py.sqrt( (x[xi]-xc)**2 + (y[yi]-yc)**2 )
            if (rm < rmlim):
                h0[xi,yi]=h0max*(1-(rm)/rmlim)        
            else:
                h0[xi,yi]=0.0      
    return h0


def plothuv(xh, yh, xu, yu, xv, yv, h, h0, u, v, filename):
